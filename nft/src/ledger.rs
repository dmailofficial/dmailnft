...
//for Dmail
impl Ledger {
    pub fn bind_alias_for_using_dmail(&mut self, token_index: TokenIndex) -> bool {
        let token_metadata = ledger()
            .tokens
            .get_mut(&token_index)
            .expect("unable to find token index");

        if token_metadata.principal == caller() {
            for token_metadata_item in ledger().tokens.values_mut() {
                if MetadataVal::Nat8Content(1)
                    == token_metadata_item.metadata_desc[0].key_val_data[1].val
                {
                    // turn all bind into 0(false)
                    token_metadata_item.metadata_desc[0].key_val_data[1].val =
                        MetadataVal::Nat8Content(0);
                    break;
                }
            }
            // turn token_index into 1(true)
            token_metadata.metadata_desc[0].key_val_data[1].val = MetadataVal::Nat8Content(1);
            return true;
        }
        false
    }
